...
import yaml
import logging
from data_collector import connect_shoonya, connect_db, get_ltp, fetch_option_chain
        ...
        if is_market_open():
            ...
            for symbol in config['trading']['symbols']:
                ...
                if live_data is None or len(live_data) < lookback:
                    logging.warning(f"Not enough data for {symbol} to generate a signal.")
                    continue

                # 2. Generate signals
                processed_data = preprocess_live_data(live_data.copy(), scaler, api, symbol, feature_columns)
                signal, confidence = generate_signal(model, processed_data, lookback)

                logging.info(f"Generated Signal for {symbol}: {signal} with confidence {confidence:.2f}")

                # 3. Filter signals
                filtered_signal = filter_signals(signal, confidence, config['trading']['confidence_threshold'])
                logging.info(f"Filtered Signal for {symbol}: {filtered_signal}")

                # 4. Risk checks and trade execution
                if filtered_signal != "Hold":
                    if len(account.positions) < config['trading']['max_positions']:
                        spot_price = get_ltp(api, 'NSE', symbol) # Underlying spot price
                        ...
                        if spot_price and option_chain:
                            ...
                            if opportunity:
                                ...
                                current_price = get_ltp(api, 'NFO', opportunity)
                                if current_price:
                                    ...
                                    if risk_per_share > 0:
                                        ...
                                        if quantity > 0:
                                            logging.info(f"Executing {filtered_signal} order for {opportunity} of {quantity} contracts.")
                                            account.place_order(opportunity, 0, filtered_signal.upper(), quantity, current_price, stop_loss_price, take_profit_price)
                                        else:
            ...
            for pos_symbol in list(account.positions.keys()):
                price = get_ltp(api, 'NFO', pos_symbol)
                if price:
